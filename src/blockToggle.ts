...
/**
 * Block state toggle for PoE filter files.
 *
 * Four states:
 *   show-custom — Show + CustomAlertSound active, PlayAlertSound commented
 *   show-system — Show + PlayAlertSound active, CustomAlertSound commented
 *   hide        — Hide + all visual effects commented
 *   disabled    — entire block commented with #
 *
 * Comment formats:
 *   Block disable:  "# " + original text  (e.g. "# Show ...", "#     Class ...")
 *   Visual comment: "#" + 6 spaces + trimmed content  (e.g. "#      PlayAlertSound 3 300")
 *
 * Toggle cycles: show-custom → show-system → hide → disabled → show-custom
 */

type BlockState = 'show-custom' | 'show-system' | 'hide' | 'disabled';
...
export class PoeFilterBlockToggle {
  constructor(context: vscode.ExtensionContext) {
    context.subscriptions.push(
      vscode.commands.registerTextEditorCommand(
        'poe-filter-best.toggleBlock',
        (editor) => this.execute(editor, 'toggle')
      ),
      vscode.commands.registerTextEditorCommand(
        'poe-filter-best.showBlockCustom',
        (editor) => this.execute(editor, 'show-custom')
      ),
      vscode.commands.registerTextEditorCommand(
        'poe-filter-best.showBlockSystem',
        (editor) => this.execute(editor, 'show-system')
      ),
      vscode.commands.registerTextEditorCommand(
        'poe-filter-best.hideBlock',
        (editor) => this.execute(editor, 'hide')
      ),
      vscode.commands.registerTextEditorCommand(
        'poe-filter-best.disableBlock',
        (editor) => this.execute(editor, 'disabled')
      ),
    );
  }

  private execute(editor: vscode.TextEditor, mode: 'toggle' | BlockState): void {
    const doc = editor.document;
    if (doc.languageId !== 'poe-filter') return;
    ...
    let targetState: BlockState;

    if (mode === 'toggle') {
      const next: Record<BlockState, BlockState> = {
  ...
  private detectState(doc: vscode.TextDocument, block: BlockRange): BlockState {
    if (block.isDisabled) return 'disabled';

    const headerText = doc.lineAt(block.start).text.trim();
    if (/^Hide\b/i.test(headerText)) return 'hide';
    ...
    for (let i = block.start + 1; i <= block.end; i++) {
      ...
      if (/^CustomAlertSound\b/i.test(trimmed)) hasCustomActive = true;
      if (/^PlayAlertSound\b/i.test(trimmed)) hasSystemActive = true;
    }

    if (hasSystemActive && !hasCustomActive) return 'show-system';
    return 'show-custom';
  }
   ...
   */
  private getActiveVisualKws(state: BlockState): Set<string> {
    switch (state) {
      case 'show-custom':
   ...
   */
  private buildHeader(currentText: string, keyword: string): string {
   ...
   */
  private getVisualKeyword(text: string): string | null {
    const trimmed = text.trim();
    let content = trimmed;
   ...
   */
  private isDisabledComment(text: string): boolean {
    return /^# /.test(text);
  }
   ...
   */
  private isInlineCommented(text: string): boolean {
    const trimmed = text.trim();
   ...
   */
  private stripBlockComment(text: string, bodyIndent: string): string {
    if (text.startsWith('# ')) {
   ...
   */
  private stripAnyComment(text: string, bodyIndent: string): string {
    const trimmed = text.trim();
  ...
  private detectBodyIndent(doc: vscode.TextDocument, start: number, end: number): string {
  ...
  findBlockRange(doc: vscode.TextDocument, lineNum: number): BlockRange | null {
  ...
  private findDisabledBlockRange(doc: vscode.TextDocument, headerLine: number): BlockRange {
    let endLine = headerLine;
    for (let i = headerLine + 1; i < doc.lineCount; i++) {
      const t = doc.lineAt(i).text.trim();
      if (t === '') continue;

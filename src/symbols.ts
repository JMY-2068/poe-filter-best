  ...
  provideDocumentSymbols(
    ...
    for (let i = 0; i < document.lineCount; i++) {
      ...
      if (headerMatch) {
        ...
        if (blockStart >= 0) {
          symbols.push(this.createBlockSymbol(
            document, blockStart, i - 1, blockType, blockSummary, blockComments
          ));
        }
    ...
    if (blockStart >= 0) {
      symbols.push(this.createBlockSymbol(
        document, blockStart, document.lineCount - 1, blockType, blockSummary, blockComments
      ));
    }

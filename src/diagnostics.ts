...
import { getKeywordDef, getAllKeywords } from './data';
...
export class PoeFilterDiagnosticsProvider {
  ...
  private validate(document: vscode.TextDocument) {
    ...
    let inBlock = false;
    const blockKeywords = new Map<string, number>();
    ...
    for (let i = 0; i < document.lineCount; i++) {
      ...
      if (inBlock) {
        ...
        if (blockKeywords.has(kwLower) && !allowDup) {
          ...
          diagnostics.push(new vscode.Diagnostic(
            range,
            `"${kw}" 在此 block 中重复定义（首次出现在第 ${firstLine + 1} 行）`,
            vscode.DiagnosticSeverity.Warning
          ));
        } else {
        ...
        if (kwMatch && !trimmed.startsWith('#')) {
          if (KNOWN_KEYWORDS.has(kwMatch[1].toLowerCase()) && !BLOCK_HEADERS.has(kwMatch[1].toLowerCase())) {
            ...
            diagnostics.push(new vscode.Diagnostic(
              range,
              `"${kwMatch[1]}" 必须在 Show/Hide block 内部使用`,
              vscode.DiagnosticSeverity.Error
            ));
          }
        }
  ...
  private validateValue(
    ...
    content: string,
    keyword: string,
    def: { valueType: string; validValues?: string[]; operators?: string[] }
  ) {
    ...
    switch (def.valueType) {
      ...
      case 'boolean': {
        if (!afterKeyword) {
          ...
          diagnostics.push(new vscode.Diagnostic(
            range,
            `"${afterKeyword}" 不是有效的值。应为: ${def.validValues.join(', ')}`,
            vscode.DiagnosticSeverity.Error
          ));
        }
        break;
      ...
      case 'icon': {
        ...
        if (parts.length < 3) {
          ...
          diagnostics.push(new vscode.Diagnostic(
            range,
            'MinimapIcon 需要: <size 0-2> <color> <shape>',
            vscode.DiagnosticSeverity.Error
          ));
        } else {

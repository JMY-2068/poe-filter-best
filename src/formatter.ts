...
interface BodyLine {
  type: 'syntax' | 'pure-comment' | 'commented-syntax';
  content: string;
...
interface ParsedBlock {
  ...
  lines: BodyLine[];  // all body lines in original order
}

// Split a trimmed line into content before # and inline comment
// Respects quoted strings — # inside quotes is not a comment
function splitInlineComment(line: string): { content: string; inlineComment: string } {
  let inQuote = false;
...
function parseFilter(text: string): { blocks: ParsedBlock[]; leadingComments: string[] } {
...
function normalizeSyntaxLine(line: string): string {
...
function formatBlock(block: ParsedBlock): string {
  const lines: string[] = [];
...
export function formatFilterText(text: string): string {
...
export class PoeFilterDocumentFormatter implements vscode.DocumentFormattingEditProvider {
...
export class PoeFilterSelectionFormatter implements vscode.DocumentRangeFormattingEditProvider {

...
提供语法高亮、智能补全、格式化、语法校验、代码片段、悬停提示、代码折叠、大纲导航、颜色预览、定义跳转、效果预览、Block 状态切换、BaseType 链接、参数选择器、音效试听和全局批量操作，让编写 POE 过滤器就像写代码一样高效。
...
- **三个 tab** — 过滤编辑 / 全局操作 / 我的过滤 / 批量改色
- **过滤编辑** — 1 级分类图片 + 2 级目录折叠 + block 预览
- **预览块** — 带实际配色的物品名标签 + 掉落图标 + 光柱图片
- **搜索框** — 固定顶部，实时过滤 block
- **点击跳转** — 点击预览块跳转到编辑器对应行（定位到顶部）
- **三档密度** — 紧凑/标准/宽松，可在设置中切换

### 🔊 Title bar 音效切换（v7 新增）

编辑器 tab 栏右侧音效图标，点击弹出下拉菜单：
...
- **音效模式切换** — 自定义/系统音效一键切换
- **全局字体 +1/-1** — 全文件 SetFontSize 统一增减（范围 12-45）
- **全局光柱开关** — 仅活跃 Show 块的 PlayEffect 一键开启/关闭

### 📂 我的过滤（v8 新增）

左侧面板「我的过滤」tab 自动管理过滤文件：
- **自动扫描** — 检测 POE1 / POE2 游戏目录下的 `.filter` 文件
- **分组展示** — 按游戏分组：流放之路 (Path of Exile) / 流放之路 降临 (Path of Exile 2)
- **文件信息** — 显示文件名、大小、修改日期
- **点击打开** — 点击文件名在编辑器中打开
- **删除文件** — 带确认弹窗的删除操作
- **刷新** — 重新扫描目录
...
- **自动分组** — 按文字色 + 背景色 + 边框色（含透明度）组合分组
- **效果预览** — 色块展示实际配色效果
- **预览按钮** — 查看该配色下所有过滤块列表，点击跳转定位
- **改色按钮** — 颜色选择器（RGB + 透明度滑块 0-255），一键批量更新同组所有块
- **安全** — 只改活跃过滤块，不改已禁用块

### ⚙️ 快速设置（v9 新增）

tab 栏右侧 ⚙️ 按钮，一键打开扩展设置页。

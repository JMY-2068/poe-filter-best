...
class PoeFilterPanel {
    ...
    resolveWebviewView(view, _resolveContext, _token) {
        ...
        view.webview.onDidReceiveMessage(async (msg) => {
            if (msg.type === 'goToBlock' && typeof msg.line === 'number') {
                ...
                const confirm = await vscode.window.showWarningMessage(`确定删除 "${name}"？此操作不可撤销。`, { modal: true }, '删除');
                if (confirm === '删除') {
                    try {
    ...
    parseBlocks(document) {
        ...
        for (let i = 0; i < document.lineCount; i++) {
            ...
            if (blockStart >= 0) {
                ...
                if (cm) {
                    textColor = { r: +cm[1], g: +cm[2], b: +cm[3], a: cm[4] ? +cm[4] : undefined };
                    continue;
                }
                const bm = content.match(/^SetBackgroundColor\s+(\d+)\s+(\d+)\s+(\d+)(?:\s+(\d+))?/i);
                if (bm) {
                    bgColor = { r: +bm[1], g: +bm[2], b: +bm[3], a: bm[4] ? +bm[4] : undefined };
                    continue;
                }
                const brm = content.match(/^SetBorderColor\s+(\d+)\s+(\d+)\s+(\d+)(?:\s+(\d+))?/i);
                if (brm) {
                    borderColor = { r: +brm[1], g: +brm[2], b: +brm[3], a: brm[4] ? +brm[4] : undefined };
                    continue;
                }
    ...
    handleBatchColor(msg) {
        const editor = vscode.window.activeTextEditor;
        if (!editor)
            return;
        const doc = editor.document;
        const key = msg.groupKey;
        ...
        const newText = msg.newTextR !== undefined ? { r: msg.newTextR, g: msg.newTextG, b: msg.newTextB, a: msg.newTextA } : null;
        const newBg = msg.newBgR !== undefined ? { r: msg.newBgR, g: msg.newBgG, b: msg.newBgB, a: msg.newBgA } : null;
        const newBorder = msg.newBorderR !== undefined ? { r: msg.newBorderR, g: msg.newBorderG, b: msg.newBorderB, a: msg.newBorderA } : null;
        const fmtColor = (r, g, b, a) => a !== undefined && a !== 255 ? `${r} ${g} ${b} ${a}` : `${r} ${g} ${b}`;
        ...
        for (const block of blocks) {
            if (block.disabled)
                continue;
            if (colorKey(block.textColor, block.bgColor, block.borderColor) !== key)
                continue;
...
function colorKey(text, bg, border) {
    const c = (v) => v ? `${v.r},${v.g},${v.b}${v.a !== undefined ? ',' + v.a : ''}` : '-';
    return `${c(text)};${c(bg)};${c(border)}`;
}
function parseColorPart(s) {
    if (s === '-')
        return null;
